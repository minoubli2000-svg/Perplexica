    ...
    <html lang="fr">
      <body className="min-h-screen w-full overflow-hidden bg-[#191F38]">
        <main className="bg-[#191F38] w-full h-full min-h-screen">
          ...
          <div className="flex items-center justify-between px-12 pt-10" style={{ background: "transparent", WebkitAppRegion: "drag" }}>
            ...
            <div className="flex items-center space-x-6" style={{ WebkitAppRegion: "no-drag" }}>
              {/* Boutons natifs Electron */}
              <button className="w-10 h-10 flex items-center justify-center rounded hover:bg-blue-900 transition"
                      onClick={() => window.electronAPI?.minimizeWindow()} title="Réduire">
                <FaMinus size={32} />
              </button>
              <button className="w-10 h-10 flex items-center justify-center rounded hover:bg-blue-900 transition"
                      onClick={() => window.electronAPI?.maximizeWindow()} title="Agrandir">
                <FaWindowMaximize size={32} />
              </button>
              <button className="w-10 h-10 flex items-center justify-center rounded hover:bg-red-600 transition"
                      onClick={() => window.electronAPI?.closeWindow()} title="Fermer">
                <FaTimes size={32} />
              </button>
            </div>
          </div>
