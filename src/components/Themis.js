...
const handleFileExtract = async (file) => {
  ...
  try {
    ...
    // Appel direct Ã  l'API extraction (Ã©vite upload sÃ©parÃ©)
    ...
    <div className="flex items-center justify-end px-1">
        ...
        {showImportModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 min-w-[400px] max-w-[600px] border border-gray-200 dark:border-gray-700 shadow-2xl">
              <h4 className="text-xl font-bold mb-4 text-gray-800 dark:text-gray-200">Importer une Question/RÃ©ponse</h4>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">RÃ©ponse:</label>
                  <textarea value={importedA} onChange={e => setImportedA(e.target.value)} className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-blue-500" rows={4} placeholder="Saisissez la rÃ©ponse..." />
                </div>
              </div>
      ...
      <aside className="flex flex-col flex-1 max-w-[270px] min-w-[200px] bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-l border-blue-200 dark:border-gray-700 py-6 px-4 space-y-4">
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-600 text-xs text-gray-500 dark:text-gray-400 space-y-1">
          ...
          <div>Historique: <strong>{history.length}</strong> entrÃ©es</div>
        </div>
      </aside>
    </div>
  );
