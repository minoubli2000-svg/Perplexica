...
const libraryApi = {
  ...
  async getStructure() {
    ...
    if (!res.ok || j.error) throw new Error(j.error || 'Erreur récupération structure');
    return j;
  },
  ...
  async extractFile(file) {
    ...
    if (!res.ok || j.error) throw new Error(j.error || 'Erreur extraction');
  ...
  async deleteFile(filename, model, subdir = null) {
    ...
    if (!res.ok || j.error) throw new Error(j.error || 'Erreur suppression');
    return j;
  },
  ...
  async renameFile(oldName, newName, model, subdir = null) {
    ...
    if (!res.ok || j.error) throw new Error(j.error || 'Renommage indisponible');
    return j;
  },
  ...
  async uploadFile(file, model, subdir) {
    ...
    if (!res.ok || j.error) throw new Error(j.error || 'Erreur upload');
    return j;
  },
};
    ...
    <div className="w-full mb-4">
      <div className="flex items-center gap-2">
        <progress
          value={percent}
          max="100"
          className="w-[95%] h-3 rounded-full [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-blue-500"
        />
...
const LibrarySidebar = ({ onStructureChange }) => {
  ...
  const handleRename = async (filename, model, subdir = null) => {
    ...
    try {
      ...
      alert(
        'Renommage indisponible côté serveur. Ajouter /api/library/rename ou renommer manuellement.',
      );
    } finally {
      setLoading(false);
    }
          ...
          {Array.isArray(files) && files.length > 0 ? (
            files.map((file) => {
              ...
              return (
                <div
                  key={name}
                  className="flex justify-between items-center py-1 px-2 bg-white dark:bg-gray-600 rounded text-xs"
                >
                  ...
                  <div className="flex items-center gap-1">
                    {size != null && (
                      <span className="text-gray-500">{Math.round(size / 1024)} KB</span>
                    )}
                    <button
                      onClick={() => handleRename(name, modelName, subdir || null)}
                    ...
                    <button
                      onClick={() => handleDelete(name, modelName, subdir || null)}
  ...
  return (
    <div className="flex flex-col h-full p-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-r border-blue-200 dark:border-gray-700">
      ...
      <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
        <div className="space-y-2">
          <select
            ...
            className="w-full px-2 py-1 text-sm border rounded bg-white dark:bg-gray-600 text-gray-800 dark:text-gray-200"
          >
          ...
          <ThemisButton
            ...
            disabled={!uploadFile || !uploadSubdir}
            loading={loading}
            size="sm"
            variant="primary"
            icon={<FaUpload />}
      ...
      <div className="flex-1 overflow-y-auto space-y-3">
        {structure &&
          Object.entries(structure).map(([modelName, modelData]) => (
            <div key={modelName} className="border rounded-lg p-3 bg-gray-50 dark:bg-gray-700">
              ...
              {selectedModel === modelName && (
                <div className="ml-2 space-y-2">{renderModelTree(modelName, modelData)}</div>
              )}
            </div>
          ))}
      </div>
...
export default function Themis() {
  ...
  useEffect(() => {
    if (role === 'general') setOnlineMode('en_ligne');
  }, [role]);
  useEffect(() => {
    const availableEngines = {
      en_ligne: ['perplexity', 'gpt'],
      hors_ligne: ['ollama', 'perplexica'],
    };
  ...
  useEffect(() => {
    try {
      localStorage.setItem('themis-history', JSON.stringify(history));
    } catch {}
  }, [history]);
  useEffect(() => {
    try {
      const saved = localStorage.getItem('themis-history');
      if (saved) setHistory(JSON.parse(saved));
    } catch {}
  }, []);
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Enter' && (e.ctrlKey || e.metaKey) && question.trim()) handleAskAI();
    };
  ...
  const handleEngineChange = (next) => {
    setEngine(next);
    if (!models[next])
      setModels((prev) => ({ ...prev, [next]: MODEL_OPTIONS[next]?.[0]?.value || '' }));
  };
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <main className="flex-1 flex flex-col items-stretch">
        ...
        <div className="flex flex-wrap items-center gap-3 px-6 py-4 border-b border-blue-200 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80">
          ...
          {/* Profil */}
          <div>
            <label className="text-xs font-semibold text-gray-700 dark:text-gray-300 mr-2">
              Profil:
            </label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="px-2 py-1 rounded border bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200"
            >
              {ROLES.map((r) => (
          ...
          <div className="mx-6 mb-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {history.slice(0, 3).map(({ question: q, answer: a }, idx) => (
                <div
                  key={idx}
                  className="p-3 bg-white dark:bg-gray-700 rounded border-l-2 border-gray-300 dark:border-gray-600"
                >
                  <div className="text-sm">
                    <strong className="text-blue-600 dark:text-blue-400">Q:</strong> {q}
                  </div>
                  <div className="text-sm mt-1">
                    <strong className="text-green-600 dark:text-green-400">R:</strong>{' '}
                    {(a || '').substring(0, 100)}...
                  </div>
                </div>
              ))}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 min-w-[400px] max-w-[600px] border border-gray-200 dark:border-gray-700 shadow-2xl">
              ...
              <div className="flex gap-3 mt-6 justify-end">
                <ThemisButton onClick={() => setShowImportModal(false)} variant="outline">
                  Annuler
                </ThemisButton>
                <ThemisButton
                  onClick={handleImportQR}
                  variant="success"
                  disabled={!importedQ.trim() || !importedA.trim()}
                >
                  Importer
                </ThemisButton>
              </div>
      ...
      <aside className="flex flex-col flex-1 max-w-[270px] min-w-[200px] bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-l border-blue-200 dark:border-gray-700 py-6 px-4 space-y-4">
        ...
        <h3 className="text-lg font-bold text-center text-blue-700 dark:text-blue-300">
          Configuration IA
        </h3>
        <div className="pt-4 border-t border-gray-200 dark:border-gray-600 text-xs text-gray-500 dark:text-gray-400 space-y-1">
          ...
          <div>
            Model: <strong>{models[engine]}</strong>
          </div>
          <div>
            Role: <strong>{role}</strong>
          </div>
          <div>
            Historique: <strong>{history.length}</strong> entrées
          </div>
        </div>
      </aside>
    </div>
